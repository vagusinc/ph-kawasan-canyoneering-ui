...
import { TItemType, TSingleResponse } from "~/types/StrapiTypes";
...
interface INavBarProps {
  ...
  banner?: TSingleResponse<TBannerType>;
}
...
const NavBar = (props: INavBarProps) => {
  ...
  const hasBanner = !!props.banner && !!props.banner.data.attributes.enabled;

  return (
    <>
      {!!hasBanner && (
        <Banner bannerText={props.banner!.data.attributes.bannerText} />
      )}
      <div
        className={`w-screen fixed flex items-center px-8 lg:px-48 ${
          hasBanner ? "py-11" : "py-8"
        } z-50 ${isElevated ? "bg-white shadow-lg py-2" : ""}`}
      >
        <div
